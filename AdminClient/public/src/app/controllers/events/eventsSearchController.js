...
(function () {
    angular
        .module("app")
        .controller("eventsSearchController", eventsSearchController);

    eventsSearchController.$inject = ['$rootScope', '$scope', '$modal', '$translate', '$q', '$timeout', 'sports', 'providers',
        'eventService', 'categoryService', 'toastr', 'eventStatus'];

    function eventsSearchController($rootScope, $scope, $modal, $translate, $q, $timeout, sports, providers,
        eventService, categoryService, toastr, eventStatus) {
        sports.data.unshift({ name: "all" });
        $scope.sports = sports.data;
        $scope.providers = providers.data;

        $scope.filter = eventService.filter[eventStatus.ACTIVE];
        $scope.events = eventService.events[eventStatus.ACTIVE];
        ...
        $scope.datePicker = {};
        $scope.datePicker.date = { startDate: moment(), endDate: moment().add(1, 'month') };
